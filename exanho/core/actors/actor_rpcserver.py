...
class RpcServer(Actor):
    ...
    def run(self, config):
        ...
        self.serv = TCPServer((config.host, config.port), self.service_class)
        ...
        if config.concurrency_degree > 0:
            if config.concurrency_type == 'process':
                ...
                raise Exception(f'The concurrency_type is "{config.concurrency_type}". There must be either "Thread" or "Process".')

...
class SleepWorker(Actor):
            ...
            if worker_config.concurrency:
                if worker_config.concurrency.kind.lower() == 'process':
                    for n in range(worker_config.concurrency.degree):
                        p = multiprocessing.Process(target=self.start_worker, args=(worker_config.module, worker_config.sleep, worker_config.appsettings, self.worker_process_terminated, self._log_queue), name=f'Process#{n}')
                        p.daemon = worker_config.concurrency.daemon
                        p.start()
                        self.workers.append(p)
                elif worker_config.concurrency.kind.lower() == 'thread':
                    for n in range(worker_config.concurrency.degree):
                        t = threading.Thread(target=self.start_worker, args=(worker_config.module, worker_config.sleep, worker_config.appsettings, self.worker_thread_terminated, None), name=f'Thread#{n}')
                ...
                t = threading.Thread(target=self.start_worker, args=(worker_config.module, worker_config.sleep, worker_config.appsettings, self.worker_thread_terminated, None))
                t.daemon = True
                t.start()
                self.workers.append(t)
    ...
    def start_worker(self, worker_module, timeout, appsettings, event, log_queue):
        if log_queue:

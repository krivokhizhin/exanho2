...
from exanho.purchbot.utils import account_manager as acc_mngr
...
class TestAccountManager(unittest.TestCase):
    ...
    def test_get_account_name(self):
        ...
        test_set = [
            ...
            SingleTestCase(initial_data=(BalAccCode.C131, 987, 555), expected='13100000009870000000555'),
        ...
        for single_test_case in test_set:
            if type(single_test_case.expected) == type and issubclass(single_test_case.expected, Exception):
                with self.assertRaises(single_test_case.expected):
                    self.assertEqual(
                        acc_mngr.get_account_name(*single_test_case.initial_data),
    ...
    def test_get_account(self):
        ...
        expected_account_name = acc_mngr.get_account_name(expected_bal_code, expected_analitic1, expected_analitic2)
        ...
        actual_acc_acount = acc_mngr.get_account(self.session, expected_bal_code, expected_analitic1, expected_analitic2, desc=expected_desc)
    ...
    def test_get_remain(self):
        acc_acount = acc_mngr.get_account(self.session, BalAccCode.C101, 777777, 666666)
        ...
        actual_remain = acc_mngr.get_remain(self.session, acc_acount)
    ...
    def test_get_remain_amount(self):
        acc_acount = acc_mngr.get_account(self.session, BalAccCode.C133)

        actual_remain_amount = acc_mngr.get_remain_amount(self.session, acc_acount)
        self.assertEqual(actual_remain_amount, Decimal('0'))

        acc_remain = acc_mngr.get_remain(self.session, acc_acount)
    ...
    def test_make_payment(self):
        dt_account1 = acc_mngr.get_account(self.session, BalAccCode.C107, 444444, 333333)
        cr_account = acc_mngr.get_account(self.session, BalAccCode.C907)

        with self.assertRaises(Error):
            acc_mngr.make_payment(self.session, dt_account1, cr_account, Decimal('-1'))

        with self.assertRaises(Error):
            acc_mngr.make_payment(self.session, dt_account1, cr_account, Decimal('100'))

        actual_record1 = acc_mngr.make_payment(self.session, dt_account1, cr_account, Decimal('100'), True)
        ...
        actual_record3 = acc_mngr.make_payment(self.session, dt_account2, cr_account, Decimal('125'), True)
        ...
        actual_record4 = acc_mngr.make_payment(self.session, dt_account3, dt_account2, Decimal('49.5'))

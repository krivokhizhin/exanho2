...
from exanho.purchbot.model import BalAccCode, AccAccount, Product, Order
...
def client_payment_acc(session:OrmSession, client_id:int, order_id:int) -> AccAccount:
    ...
    if product_code == 'QUE_PAR_ACT':
        return util.get_account(session, BalAccCode.C301, client_id, order_id, desc=desc)

    if product_code == 'QUE_PAR_HIS':
        return util.get_account(session, BalAccCode.C302, client_id, order_id, desc=desc)

    if product_code == 'SUB_PAR':
        return util.get_account(session, BalAccCode.C331, client_id, order_id, desc=desc)

    if product_code == 'REP_PAR_ACT':
        return util.get_account(session, BalAccCode.C361, client_id, order_id, desc=desc)

    if product_code == 'REP_PAR_HIS':
        return util.get_account(session, BalAccCode.C362, client_id, order_id, desc=desc)
...
def client_promo_payment_acc(session:OrmSession, client_id:int, order_id:int) -> AccAccount:
    order = session.query(Order).get(order_id)
    product_code = session.query(Product.code).filter(Product.id == order.product_id).scalar()
    desc=f'Промо-счет оплаты услуги {product_code} для клиента id={client_id}'

    if product_code == 'QUE_PAR_ACT':
        return util.get_account(session, BalAccCode.C701, client_id, order_id, desc=desc)

    if product_code == 'QUE_PAR_HIS':
        return util.get_account(session, BalAccCode.C702, client_id, order_id, desc=desc)

    if product_code == 'SUB_PAR':
        return util.get_account(session, BalAccCode.C731, client_id, order_id, desc=desc)

    if product_code == 'REP_PAR_ACT':
        return util.get_account(session, BalAccCode.C761, client_id, order_id, desc=desc)

    if product_code == 'REP_PAR_HIS':
        return util.get_account(session, BalAccCode.C762, client_id, order_id, desc=desc)

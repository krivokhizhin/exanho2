...
def prepare_list_to_dict(list_obj:list, *obj_types) -> list:
    result = list()

    for item in list_obj:
        if item or item == 0:
            if isinstance(item, obj_types):
                result.append(prepare_dict_to_dict(item.__dict__, *obj_types))
            elif isinstance(item, dict):
                result.append(prepare_dict_to_dict(item, *obj_types))
            elif isinstance(item, list):
                result.append(prepare_list_to_dict(item))
            else:
                result.append(item)

    return result

def convert_content_to_base64(content:str) -> str:
    base64_bytes = content.encode('ascii')
    content_bytes = base64.b64encode(base64_bytes)
    return content_bytes.decode('ascii')

def convert_base64_to_content(content:str) -> str:
    base64_bytes = content.encode('ascii')
